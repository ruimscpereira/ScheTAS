...
def chave_ordenacao_personalizada(linha):
    """Ordenação rigorosa: Responsabilidades (Radiologia Convencional -> Tomografia Computorizada -> Ressonância Magnética -> Ecografia)
    e Turnos internos (M3 -> T -> M12 -> M75 -> T24)."""
    try:
...
# -----------------------------------------------------------------------------
# TAB 3: NECESSIDADES MENSAIS (COM ORDENAÇÃO E CORES GARANTIDAS)
# -----------------------------------------------------------------------------
with tabs[2]:
    ...
    if turnos_ativos.empty:
        ...
        # Obter os slots ordenados rigorosamente
        ...
        # Garante a criação e a reordenação estrita das colunas da matriz
        if not isinstance(df_nec_mes, pd.DataFrame) or list(df_nec_mes.columns) != colunas_turnos or len(df_nec_mes) != num_dias:
            ...
            # Reordena explicitamente para garantir que cumpre o padrão estipulado
        ...
        # Rótulo limpo: CÓDIGO DO TURNO + (RESPONSABILIDADE)
