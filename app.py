...
if "limites_jornadas_12h" not in st.session_state:
    ...
    for nome in st.session_state.trabalhadores:
        st.session_state.limites_jornadas_12h.setdefault(nome, LIMITE_JORNADAS_12H_PADRAO)

if "tipos_contrato" not in st.session_state:
    st.session_state.tipos_contrato = {nome: TIPO_CONTRATO_PADRAO for nome in st.session_state.trabalhadores}
else:
    for nome in st.session_state.trabalhadores:
        st.session_state.tipos_contrato.setdefault(nome, TIPO_CONTRATO_PADRAO)

if "horas_diarias" not in st.session_state:
    st.session_state.horas_diarias = {nome: HORAS_DIARIAS_PADRAO for nome in st.session_state.trabalhadores}
else:
    for nome in st.session_state.trabalhadores:
        st.session_state.horas_diarias.setdefault(nome, HORAS_DIARIAS_PADRAO)

if "indisponabilidades" not in st.session_state:
    st.session_state.indisponabilidades = {}
...
with tabs[0]:
    ...
    st.caption("Associe a cada trabalhador as responsabilidades e os turnos que está habilitado a fazer.")
    ...
    with col2:
        ...
        if st.session_state.trabalhadores:
            for i, nome in enumerate(st.session_state.trabalhadores):
                ...
                c1.caption(f"Jornadas de 12h: {'tem preferência' if preferencia_12h else 'sem preferência'} · limite: {limite_12h}/mês")
                ...
                if c3.button("Remover", key=f"del_{nome}"):
                    ...
                    st.session_state.indisponabilidades.pop(nome, None)
                ...
                with st.expander("Editar responsabilidades e turnos", expanded=False):
                    ...
                    limite_12h_editado = st.number_input("Limite mensal de jornadas de 12 horas", min_value=0, max_value=31, value=int(st.session_state.limites_jornadas_12h.get(nome, LIMITE_JORNADAS_12H_PADRAO)), step=1, help="Defina 0 se o trabalhador não puder fazer jornadas M12 + T24.", key=f"editar_limite_12h_{nome}")
                    ...
                    if st.button("Guardar competências", key=f"guardar_competencias_{nome}"):
...
with tabs[3]:
    ...
    st.caption("Preencha diretamente a tabela por trabalhador e por dia. Uma célula vazia significa disponibilidade sem preferência, F significa folga.")
    ...
    st.session_state.preferencias_turnos = st.data_editor(preferencias_com_fins_de_semana, width="stretch", num_rows="fixed", column_config={dia: st.column_config.SelectboxColumn(dia, options=opcoes_tabela, required=False, help="Escolha F para folga ou um código de turno.") for dia in dias_do_mes}, key="editor_preferencias_turnos")

# Tab 5
with tabs[4]:
    st.header("Gerador de Escala Automática")
    st.caption("O gerador pode atribuir M12 e T24 ao mesmo trabalhador no mesmo dia (08:00 às 20:00).")
    if st.button("⚡ Gerar Escala Optimizada", type="primary"):
        ...
        if not st.session_state.trabalhadores or not trabalhadores:
            ...
            # Descanso noturno (atravessa meia-noite)
            for trabalhador in trabalhadores:
            ...
            if pares_jornada:
                model.Maximize(1_000_000 * sum(pares_jornada_preferidos) + 1_000 * sum(pares_jornada) + sum(pares_responsabilidades_diferentes))
            ...
            if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
                ...
                df_resultado = pd.DataFrame.from_dict(dados_escala, orient="index", columns=colunas_dias)
                ...
                st.download_button("📥 Descarregar Escala (CSV)", csv, "escala_mensal.csv", "text/csv")
            else:
                st.error("Não foi possível encontrar uma solução válida com as restrições, necessidades e trabalhadores disponíveis.")
